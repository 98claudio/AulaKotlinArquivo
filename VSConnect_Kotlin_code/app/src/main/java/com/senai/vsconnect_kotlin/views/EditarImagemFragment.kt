...
class EditarImagemFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val sharedPreferences = requireContext().getSharedPreferences( "idUsuario",
    ...
    private fun buscarUsuarioPorID(idUsuario: String){
        endpoints.buscarUsuarioPorID(UUID.fromString(idUsuario)).enqueue(object :Callback<JsonObject>{
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                ...
                val imageViewPerfilUsuario = JSONObject(response.body().toString()).getString("url_img")

                val urlImagem = "http://172.16.52.93:8099/img/" + imageViewPerfilUsuario
